...
**The 5 card-grid views are one shared component.** `Download.vue`, `External.vue`, `Infographics.vue`, `Internal.vue`, and `PDFMaps.vue` are all thin wrappers around `src/components/CardGrid.vue`, which takes a `type` prop (passed to `getItems`), an optional `img-class` prop for the couple of pages with slightly different thumbnail styling, an optional `wrapped` boolean for the pages that wrap their content in a full-viewport-height div, and a `#blurb` slot for the page's intro paragraph. To add a new card-grid page, add a new wrapper view following the existing ones as a template — don't hand-roll another grid.
...
**Styling** is Tailwind CSS v4 via `@tailwindcss/vite` (see `vite.config.js`), imported once in `src/index.css`/`src/style.css` with `@import "tailwindcss"`. Views style almost entirely with Tailwind utility classes inline in templates, including one-off colors via arbitrary-value syntax (`bg-[#08244c]`, `text-[#f8f8f8]`) rather than inline `style=` attributes. No `tailwind.config.js` is present — v4's CSS-first config means there's nothing to check there.
